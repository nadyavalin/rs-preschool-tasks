console.log(`Оцениваю свою работу на максимальные 200 баллов - все пункты выполнены:
...
iv. Если анимация карусели не успела завершиться, при этом была нажата следующая кнопка, то картинка не зависает в промежуточном состоянии. +2
v. На экране шириной 768px доступно 4 других скрытых картинки. Для этого меняем разрешение и перезагружаем страницу. Теперь доступных перемещений становится 5. +2
vi. Неактивными становятся не только выделенные кнопки, но и стрелочки на границах карусели. +2

- Слайдер в блоке Favorites (+23)
i. "Слайдер" реагирует на нажатие кнопок панели навигации и происходит анимация затухания и проявления. +15 !!!(да, но в обратную сторону затухание не происходит, только проявление, можно попробовать переделать)!!!
...
iii. Анимация может быть прервана следующим нажатием на кнопку выбора поры года, но при этом анимация не застывает в промежуточном состоянии. Если анимация не была прервана следующим нажатием кнопки, то она отрабатывает до конца. +2
iv. Во время анимаций высота блока Favorites не меняется. +2 !!!(не уверена)!!!
...
i. Нажатие на иконку пользователя в хедере открывает меню, которое находится под иконкой таким образом, что правый верхний угол меню находится в той же точке, что и нижний правый угол контейнера с иконкой в хедере. Меню под иконкой. +2
ii. На разрешении 768px, при открытом бургер-меню, меню авторизации закрывается и открывается. +2
...
v. При нажатии на крестик в углу окна, или на затемнённую область вне этого окна, оно закрывается. +2
vi. В данном случае, ограничения по полям - все поля должны быть не пустыми. +2 (кнопка не активна, пока хоть одно поле пустое)
...
i. Блок Digital Library Cards. Если введённые имя и номер карты совпадают с данными пользователя, то отображается панель с информацией, вместо кнопки Check the card на 10 секунд. +2
...
ii. При нажатии на любую кнопку Buy, после покупки абонемента, меняет вид кнопки на неактивную Own, добавляя единицу к счетчику книг в профиле. +2
